...
class Upnp:
    ...
    def initSockets(self, ip, port, iface):
        ...
        try:
            ...
            if self.IFACE is not None:
                ...
                self.ssock.setsockopt(SOL_SOCKET, 25, struct.pack("%ds" % (len(self.IFACE) + 1,), self.IFACE))
                self.csock.setsockopt(SOL_SOCKET, 25, struct.pack("%ds" % (len(self.IFACE) + 1,), self.IFACE))
    ...
    def send(self, data, sock):
        ...
        if not sock:
            sock = self.csock
        try:
            sock.sendto(bytes(data.encode('utf-8')), (self.ip, self.port))
            return True
        except Exception as e:
        ...
        try:
            ...
            while True:
                ...
                if not data:
                    ...
                    soapResponse += data
                    if self.soapEnd.search(soapResponse.lower()) is not None:
                        break
                    sock.close()
